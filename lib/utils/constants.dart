import 'package:google_maps_flutter/google_maps_flutter.dart';
const String appName = "Duma Clone";

//SENSITIVE
const String appId = "YOUR_APP_ID";
String iQKey = "YOUR_IQ_KEY";


//
const String requirement =
    r"^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?\d)(?=.*?[!@#$&*?~%^]).{8,}$";
const LatLng initialPosition = LatLng(-3.386943, 29.371636);


//Preferences
const String langSet = "LANG";
const String isConnected = "CONNECTED";

//Assets
const String afripayImage = "assets/afripay.png";
const String errorInternetImage = "assets/error_internet.json";
const String welcomeImage = "assets/welcome.json";
const String noLocationFound = "assets/no_location.png";
const String taxi = "assets/taxi.png";
const String package = "assets/package.png";
const String personImage = "assets/person.png";
const String driver = "assets/driver.png";


//PASS
const String originMap = "originMap";
const String destinationMap = "destinationMap";
